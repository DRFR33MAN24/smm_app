...
class UsersController extends Controller
{
    ...
    public function transaction($id)
    {
        ...
        $userid = $user->id;
        $transaction = $user->transactions()->paginate(config('basic.paginate'));
    ...
    public function userUpdate(Request $request, $id)
    {
        ...
        $user->f_name = $userData['firstname'];
        $user->l_name = $userData['lastname'];
 
        $user->email = $userData['email'];
        $user->phone = $userData['phone'];
 
        $user->status = ($userData['status'] == 'on') ? 0 : 1;
        $user->is_reseller = ($userData['is_reseller'] == 'on') ? 0 : 1;
    ...
    public function userBalanceLBPUpdate(Request $request, $id)
    {
        $userData = Purify::clean($request->all());
        if ($userData['lbalance'] == null) {
            ...
            if ($userData['add_statuslbp'] == "1") {
                $user->lbalance += $userData['lbalance'];
                $user->save();

                $transaction = new Transaction();
                $transaction->user_id = $user->id;
                $transaction->trx_type = '+';
                $transaction->amount = $userData['lbalance'];
                ...
                $transaction->save();
                $msg = [
                    'description'=>getAmount($userData['lbalance']).'LBP added to your balance.',
                    'title'=>'Balance Update',
                    'image'=>''
                ];
                Helpers::send_push_notif_to_device($user->cm_firebase_token, $msg);
                ...
                if ($userData['lbalance'] > $user->lbalance) {
                ...
                $transaction->save();
                $msg = [
                    'description'=>getAmount($userData['lbalance']).'LBP deducted from your balance.',
                    'title'=>'Balance Update',
                    'image'=>''
                ];
                Helpers::send_push_notif_to_device($user->cm_firebase_token, $msg);
    ...
    public function userBalanceUpdate(Request $request, $id)
    {
        ...
        if ($userData['balance'] == null) {
            ...
            if ($userData['add_status'] == "1") {
                ...
                $transaction->save();
                $msg = [
                    'description'=>getAmount($userData['balance']).'$ added to your balance.',
                    'title'=>'Balance Update',
                    'image'=>''
                ];
                Helpers::send_push_notif_to_device($user->cm_firebase_token, $msg);
                ...
                $transaction->save();
                $msg = [
                    'description'=>getAmount($userData['balance']).'$ deducted from your balance.',
                    'title'=>'Balance Update',
                    'image'=>''
                ];
                Helpers::send_push_notif_to_device($user->cm_firebase_token, $msg);
