                    ...
                    @foreach($group["SERVICES"] as $service)
                        <tr>
                            <td class="text-center check-box-width-50">
                                <input type="checkbox" id="chk-{{@$service['SERVICEID']}}"
                                       class="form-check-input row-tic tic-check row-tic-{{@$service['SERVICEID']}}"
                                       name="check" value="{{ @$service['SERVICEID'] }}"
                                       data-id="{{ $service['SERVICEID'] }}">
                                <label for="chk-{{@$service['SERVICEID']}}"></label>
                            </td>
                            <td class="text-center">{{@$service["SERVICEID"]}}</td>
                            <td class="text-center">
                                <a href="javascript:void(0)" data-container="body"  data-toggle="popover" data-placement="top" data-content="{{@$service['SERVICENAME']}}">
                                    {{\Str::limit(@$service["SERVICENAME"], 60)}}
                                </a></td>
                            <td class="text-center">{{ @$group["GROUPNAME"] }}</td>
                            <td class="text-center"><span class="text-dark">{{ @$service['CREDIT'] }}</span> {{$provider->currency}}</td>
                                ...
                                <div class="dropdown show dropup">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
                                        <a href="javascript:void(0)" class="dropdown-item import-single" data-toggle="modal"
                                           ...
                                           data-route="{{ route('admin.api.service.import',['id'=>@$service['SERVICEID'],'name'=>@$service['SERVICENAME'],'category'=>$group['GROUPNAME'],'rate'=>$service['CREDIT'],'min'=>@$service['MINQNT'],'max'=>@$service['MAXQNT'], 'provider'=>@$provider->id,'custom_fields'=>@$service['Requires.Custom'] ?? 'IMEI']) }}">
