                    ...
                    <tr>
                        ...
                        <td data-label="@lang('User Name')">
                            <a href="{{route('admin.user-edit',$row->user_id)}}" target="_blank">
                                @lang($row->user->f_name)  @lang($row->user->l_name)
                            </a>
                        </td>
                        <td data-label="@lang('Amount')"> <span class="text-{{($row->trx_type == "+") ? 'success' :'danger'}}">{{$row->currency}}{{' '}}{{getAmount($row->amount, config('basic.fraction_number'))}}</span></td>
